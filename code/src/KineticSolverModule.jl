...
using ProgressMeter, MyIOModule, DelimitedFiles, LinearAlgebra, Dates, Revise, PyPlot
...
export setTestCaseInitialCondition!,computeHenyeyGreensteinScatteringMatrix,solveWithSweeping!,solveRSNWithSweeping!,sweepingWithKernels,vis,methods!
...
mutable struct KineticSolver
    ...
    function KineticSolver(configfilename::String)
        ...
        allowed = ["nx", "ny", "cfl", "quadraturetype", "quadratureorder",
                   "convolutionmagnitude", "convolutionwidth", "convolutiontoscattering",
                   "testcaseid", "rotationmagnitude",
                   "periodicXflag", "periodicYflag", "dim1flag", "whichrank"]
        ...
        if dim1flag == true ## 1D FLAG FORCES THESE THINGS:
            ny = 3; periodicYflag = true;    ThisICType = "dim1"
        ...
        x0,x1,y0,y1,tEnd,SigmaS,SigmaT,Source,psi0 = getProblemSpecificParameters(nx,ny,nquadpoints,ThisICType,Q.pointsxyz)
        ...
        new(x0,x1,y0,y1,dx,dy,nx + 4,ny + 4,# spatial
            norder,nquadpoints,             # quadrature
            rangex,rangey,rangequad,        # range
            ThisQuadratureType, Q,          # quadrature
            ...
            periodicXflag, periodicYflag, # are boundaries periodic?
            dim1flag,                     # 1d
            rank,                         # when performing low rank computations
            SigmaS,SigmaT,                # Sigma
            ThisICType,Source,psi0,       # IC/Q/PHI0
