...
function sweeping(KS::KineticSolver, psi::Array{Float64,3},Source::Array{Float64,3},ScatteringKernel::Array{Float64,2})
    ...
    for k=1:KS.nSweepingSteps
		...
		print(" ")
        println(k)
        @inbounds for j=KS.rangex,i=KS.rangey
            rhs[:,i,j] = Source[:,i,j] + KS.convolutionmagnitude.*KS.kernelTimesWeightsSparse*(psi[:,i,j])
        end
...
function sweepingWithKernels(KS::KineticSolver, psi::Array{Float64,3},Source::Array{Float64,3},asKernel::Array{Float64,2},physKernel::Array{Float64,2})
    rhs = zeros(KS.nquadpoints,KS.nx,KS.ny)
	io = open("mytestfile.txt", "a")      
	#println("")
	#println("sweeping is called")
    ...
    for k=1:KS.nSweepingSteps
        ...
        println(k)
        @inbounds for j=KS.rangex,i=KS.rangey
            rhs[:,i,j] = Source[:,i,j] + KS.SigmaS[i,j].*physKernel*psi[:,i,j] + KS.convolutionmagnitude.*asKernel*psi[:,i,j];
        end
        ...
        # check residual
        println("-> Sweeping norm: ",norm(psi-psiOld)," ; constraction: ",KS.contractionEstimate)
        if norm(psi-psiOld)*KS.contractionEstimate < KS.epsSweeping
			writedlm(io,k)
			close(io)
            return psi
...
function solveFluxSweepingBiggerStencil!(KS::KineticSolver, psi::Array{Float64,3},rhs::Array{Float64,3})
    ...
    # PosPos
    @inbounds for j=KS.rangex,i=KS.rangey, q = idxPosPos
        s0 = psi[q,i,j-3]
        s1 = psi[q,i,j-2]
        s2 = psi[q,i,j-1]

        fluxExplY = KS.Q.pointsxyz[q,2]./KS.dy .* ( - s2 + 1/4 * s1 )
        fluxExplY = fluxExplY - KS.Q.pointsxyz[q,2]./KS.dy .* ( (7/4)*s2 - s1 + 1/4 * s0 )
        ...
        fluxExplX = fluxExplX - KS.Q.pointsxyz[q,1]./KS.dx .* ( (7/4)*s2 - s1 + 1/4 * s0 )
        ...
        psi[q, i, j]  = KS.dt .*( - flux + rhs[q,i,j])/(1+KS.dt .* ( KS.SigmaT[i,j]+fluxImplicit))
    end
    #PosNeg
    @inbounds for j=Iterators.reverse(KS.rangex),i=KS.rangey,q = idxPosNeg
        s0 = psi[q,i,j+1]
        s1 = psi[q,i,j+2]
        s2 = psi[q,i,j+3]
    ...
    # NegPos
    @inbounds for j=KS.rangex,i=Iterators.reverse(KS.rangey),q = idxNegPos
        s0 = psi[q,i,j-3]
        s1 = psi[q,i,j-2]
        s2 = psi[q,i,j-1]
    ...
    # NegNeg
    @inbounds for j=Iterators.reverse(KS.rangex),i=Iterators.reverse(KS.rangey),q = idxNegNeg
        s0 = psi[q,i,j+1]
        s1 = psi[q,i,j+2]
        s2 = psi[q,i,j+3]
...
function solveFluxSweepingFirstOrder!(KS::KineticSolver, psi::Array{Float64,3},rhs::Array{Float64,3})
    # computes the numerical flux over cell boundaries for each ordinate
    # for faster computation, we split the iteration over quadrature points
    # into four different blocks: North West, Nort East, Sout West, South East
    # this corresponds to the direction the ordinates point to
    ...
    @inbounds for j=KS.rangex,i=KS.rangey, q = idxPosPos
        ...
        flux = KS.Q.pointsxyz[q,1] ./KS.dx .* (-westflux) + KS.Q.pointsxyz[q,2]./KS.dy .* (-southflux)
        psi[q, i, j]  = KS.dt .*( - flux + rhs[q,i,j])/(1+KS.dt .* ( KS.SigmaT[i,j]+fluxImpllicit));
    end
    #PosNeg
    @inbounds for j=Iterators.reverse(KS.rangex),i=KS.rangey,q = idxPosNeg
        s1 = psi[q,i,j-1]
        ...
        fluxImpllicit = KS.Q.pointsxyz[q,1] ./KS.dx +
        KS.Q.pointsxyz[q,2]./KS.dy .* (-1)
        flux = KS.Q.pointsxyz[q,1] ./KS.dx .* (-westflux) +
    ...
    @inbounds for j=KS.rangex,i=Iterators.reverse(KS.rangey),q = idxNegPos
        ...
        fluxImpllicit = KS.Q.pointsxyz[q,1] ./KS.dx .* (-1) + KS.Q.pointsxyz[q,2]./KS.dy
        flux = KS.Q.pointsxyz[q,1] ./KS.dx .* (eastflux) +
    ...
    @inbounds for j=Iterators.reverse(KS.rangex),i=Iterators.reverse(KS.rangey),q = idxNegNeg
        ...
        fluxImpllicit = KS.Q.pointsxyz[q,1] ./KS.dx .* (-1) +
        KS.Q.pointsxyz[q,2]./KS.dy .* (-1)
        flux = KS.Q.pointsxyz[q,1] ./KS.dx .* (eastflux) +
        KS.Q.pointsxyz[q,2]./KS.dy .* (northflux)
        psi[q, i, j]  = KS.dt .*( - flux + rhs[q,i,j])/(1+KS.dt .* ( KS.SigmaT[i,j]+fluxImpllicit));
