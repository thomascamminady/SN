function  getProblemSpecificParameters(nx,ny,nquadpoints,ThisICType,pointsxyz)
	...
	elseif ThisICType=="beam"
		...
		beamdirection = [0,1,0]
		Source = setInitialConditionsBeam(nx,ny,nquadpoints,dx,dy,x0,y0,pointsxyz,beamdirection)
	...
	elseif ThisICType=="checkerboard"
		...
		for j=3:nx+2
			for i=3:ny+2
				...
				cx = ceil(x);
				cy = ceil(y);
				g = float(ceil((cx+cy)/2.0)*2.0==(cx+cy))*((1.0<cx)&&(cx<7.)&&(1.0<cy)&&((cy-2.0*abs(cx-4.0))<4.0));
...
function setInitialConditionsGaussian(nx,ny,nq,dx,dy,domainx0,domainy0) 
    ...
    s2 = 0.03^2
    floor = 1e-4
    f(x,y) = max(floor,1.0/(4.0*pi*s2) *exp(-((x-x0)*(x-x0)+(y-y0)*(y-y0))/4.0/s2));
    for j=3:nx+2
        for i=3:ny+2   
...
function setInitialConditionsBeam(nx,ny,nq,dx,dy,domainx0,domainy0,quadpoints,beamdirection) 
    # init to zero
